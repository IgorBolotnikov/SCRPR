...
RESET_PASSWORD_REQUEST_VALID_FORM = {"email": "testuser1@testemail.com"}
...
RESET_PASSWORD_REQUEST_INVALID_FORM_1 = {"email": "notavalidemail"}
RESET_PASSWORD_REQUEST_INVALID_ERRORS_1 = {"email": EMAIL_ERROR_1}
...
class TestCaseWithDatabase(TestCase):
    ...
    def setUpTestData(cls):
        test_user1 = User.objects.create(
            email="testuser1@testemail.com", username="TestUser1",
        )
        test_user2 = User.objects.create(
            email="testuser2@testemail.com", username="TestUser2"
        )
...
class TestLoginView(TestCaseWithDatabase, GetResponseMixin):
    def setUp(self):
        self.url = "/auth/login/"
    ...
    def test_failed_login_attempt(self):
        for form_sample in self.invalid_form_samples:
            ...
            self.assertEqual(response.status_code, 200)
            self.assertEqual(
                response.context["form"].errors["__all__"][0], LOGIN_ERROR
            )
...
class TestRegisterView(
    TestCaseWithDatabase, GetResponseMixin, FormValidationMixin
):
    def setUp(self):
    ...
    def test_user_creation(self):
        self.client.post(self.url, self.valid_form_sample)
        obj = User.objects.filter(pk=3).first()
        self.assertEqual(obj.username, self.valid_form_sample["username"])
        self.assertEqual(obj.email, self.valid_form_sample["email"])
        self.assertTrue(obj.image is not None)


class TestResetPasswordRequestView(
    TestCaseWithDatabase, GetResponseMixin, FormValidationMixin
):
    def setUp(self):
    ...
    def test_nonexisting_email_response(self):
        response = self.client.post(self.url, {"email": "useremail@mymail.com"})
        self.assertEqual(response.status_code, 302)


class TestResetPasswordRequestDoneView(TestCaseWithDatabase, GetResponseMixin):
    def setUp(self):
        self.url = "/auth/reset_password_done"
        self.template_name = "authentication/reset_password_done.html"


class TestResetPasswordView(TestCaseWithDatabase, GetResponseMixin):
    def setUp(self):
        self.url = ""
    ...
    def test_get_response(self, mock_sendgrid):
        response = self.client.post(
            "/auth/reset_password", RESET_PASSWORD_REQUEST_VALID_FORM
        )
...
class CompleteResetPasswordView(TestCaseWithDatabase, GetResponseMixin):
    def setUp(self):
...
class TestDeleteAccountView(
    TestCaseWithDatabase, GetResponseMixin, ObjectDeletionWithRedirectMixin
):
    def setUp(self):
