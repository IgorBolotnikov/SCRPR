...
class FavoriteGameQuery(models.Model):
    ...
    def __str__(self):
        return f'Query: "{self.title}" by {self.account.username}, {self.notification_freq}'

    @property
    def details(self):
        details = ''
        details += f', {NUMERIC_MIN_LABEL}: {self.price_min} UAH' if self.price_min else ''
        details += f', {NUMERIC_MAX_LABEL}: {self.price_max} UAH' if self.price_max else ''
