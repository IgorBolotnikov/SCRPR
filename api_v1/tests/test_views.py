...
from scrpr.models import Comment
...
class PaginationMixin:
    ...
    def test_paginated_results(self):
        ...
        if self.models_owner:
            mixer.cycle(self.objects_num).blend(self.model_class, account=user)
        else:
            mixer.cycle(self.objects_num).blend(self.model_class)
        ...
        assert pagination["prev_page"] is None, "Should not have previous page"
...
class TestCommentView:
    def test_post_response(self):
        post_boby = {"name": "Myname", "comment": "Mycomment"}
...
class TestFavoritesGamesListView(PaginationMixin):
    ...
    def test_get_not_author_response(self):
        request = APIRequestFactory().get("/")
        user = mixer.blend("authentication.User")
        mixer.blend("scrpr.FavoriteGameQuery")
        force_authenticate(request, user=user)
        response = self.view_class.as_view({"get": "retrieve"})(request, pk=1)
        assert (
...
class TestFavoritesJobsListView(PaginationMixin):
    ...
    def test_post_response(self):
        ...
        force_authenticate(request, user=user)
        response = self.view_class.as_view({"post": "create"})(request)
        assert response.status_code == 201, "Should return 200 OK"
