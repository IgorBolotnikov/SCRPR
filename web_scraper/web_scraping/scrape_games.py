...
class PSStoreScraper(ScraperBase):
    def _get_url(self, page_num=1, query_params=None):
        ...
        price_min = (
            int(query_params.get("price_min")) * 100
            if query_params.get("price_min")
            else DEFAULT_MIN_PRICE
        )
        price_max = (
            int(query_params.get("price_max")) * 100
        ...
        price_min, price_max = (
            (0, 0) if query_params.get("free") else (price_min, price_max)
        )
        self.params = {
            "query": title,
        ...
        if title:
            ...
            # In this case page nums are simply used
            # to iterate through each link.
    ...
    def _get_games_list(page):
        ...
        for game_div in game_divs:
            if game_div.find("h3", class_=PRICE_CLASSES[0]) or game_div.find(
                "h3", class_=PRICE_CLASSES[1]
            ):
    ...
    def _get_game_title(game):
        title = (
            game.find("div", class_="grid-cell__title").find("span").get_text()
        )
        parsed_title = ""
        for char in title:
            if char != "\\":
    ...
    def _get_game_image(game):
        images = game.find("div", class_="product-image__img--main").img[
            "srcset"
        ]
