...
class ScraperBase:
    async def scrape_job_website(self, location, page_num, query_params=None):
        ...
        return {"object_list": self.output, "last_page": self.last_page_num}

    # @timer
    def scrape_game_website(self, page_num, query_params=None):
        self.output = []
        asyncio.run(
            self._scrape_game_pages(
    ...
    def _filter_games_output(
        self, discount_filter=False, psplus_filter=False, free=False
    ):
        if discount_filter:
            self.output = filter(
                lambda game: game.get("initial_price") is not None, self.output
            )
        if psplus_filter:
            self.output = filter(
                lambda game: game.get("psplus_price") is not None, self.output
            )
        if free:
        ...
        async with aiohttp.ClientSession() as session:
            while page_num <= last_page_num:
                async_task = asyncio.create_task(
                    self._scrape_game_page(
                        self._get_url(page_num, query_params), page_num, session
                    )
                )
